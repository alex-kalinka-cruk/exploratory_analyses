...
```{r}
pl_gc %<>%
  mutate(Control_Plasmid.baseline.logFC = lfc.bl$Control_Plasmid.logFC.median[match(sgRNA,lfc.bl$sgRNA)],
         Control_Plasmid.day7.logFC = lfc.d7$Control_Plasmid.logFC.median[match(sgRNA,lfc.d7$sgRNA)],
         Control_Plasmid.day14.logFC = lfc.d14$Control_Plasmid.logFC.median[match(sgRNA,lfc.d14$sgRNA)])

pl <- ggplot(pl_gc %>%
               dplyr::filter(between(GC_percent,20,75)),
             aes(GC_percent, Control_Plasmid.baseline.logFC, group=GC_percent)) + 
  geom_point() + 
  geom_boxplot(notch=T) +
  geom_smooth() +
  ylim(-3,1.5) +
  geom_hline(yintercept = 0, linetype="dashed") +
  ggtitle("HT-29 QC screen: logFC BL vs GC content")
