...
Defined as sgRNAs that don't have gene IDs in the `EXONE` column of the original Kusa v3 file and are missing in the Plasmid count files (entered as '.'), but have valid gene IDs in their sgRNA IDs. Several genes in here have all 6 of their sgRNAs present (see sgRNA counts below).

```{r}
orphans <- suppressMessages(suppressWarnings(read_table2(file.path(base_path.data,"cleanr.tsv")))) %>%
  filter(EXONE == ".") %>%
  mutate(gene = gsub("^(\\S+?)_.*$","\\1",GENES))
