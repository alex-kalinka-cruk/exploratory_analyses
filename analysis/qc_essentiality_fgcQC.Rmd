...
#' make_AUROC_table
#'
#' Makes an AUROC `gt` table for html printing.
#'
#' @param data A data frame containing the following columns: `True_Positive_Rate`, `False_Positive_Rate`, and `gene_set`.
#' @param subtitle A subtitle for the table.
#'
#' @return A data frame of AUROCs.
make_AUROC_table <- function(data, subtitle){
  tryCatch({
    data_gt <- data %>%
      gt::gt() %>%
      gt::tab_header(title = "", subtitle = subtitle)
  },
  error = function(e) stop(paste("unable to make AUROC gt table:",e))
  )
  return(data_auroc)
}
